...
fetch('PrivateTransactions.json')
    ...
    .then(text => {
        ...
        // Filter the data to include only rows where Consumer matches the userID
        const filteredData = data.filter(transaction => transaction.Distributor === userID);

        if (filteredData.length === 0) {
            console.log("No transactions found for this Consumer.");
            const table = document.getElementById("transactionsTable");
            table.innerHTML = `
                <tr>
                    <td colspan="7" style="text-align: center;">No transactions found for Consumer: ${userID}</td>
                </tr>
            `;
            return;
        }
