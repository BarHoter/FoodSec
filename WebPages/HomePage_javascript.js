// Embedded JSON data directly in JavaScript
const idsData = {
  ...
  function checkUserID() {
    ...
    if (userID === "FoodGod") {
        window.location.href = "GodNavigiation.html"; // Redirect to GodPage.html
    } else if (userID) {
        localStorage.setItem("userID", userID); // Store the userID in localStorage
        window.location.href = "NavigationPage.html"; // Redirect to NavigationPage.html
    } else {
        document.getElementById("error-message").textContent = "Please enter a valid User ID.";
    }
}
